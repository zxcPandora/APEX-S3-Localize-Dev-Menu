...
void function InitDevMenu( var newMenuArg )
{
		...
		AddMenuFooterOption( menu, LEFT, BUTTON_B, true, Localize("#Back"), Localize("#Back") )
		AddMenuEventHandler( menu, eUIEvent.MENU_NAVIGATE_BACK, BackOnePage_Activate )
		AddMenuFooterOption( menu, LEFT, BUTTON_Y, true, "%[Y_BUTTON|]% "+Localize("#RepeatCommand"), Localize("#RepeatCommand"), RepeatLastCommand_Activate )
...
void function UpdateDevMenuButtons()
{
	...
	// Title:
	{
		...
		if ( titleText == "" )
			titleText = (Localize("#MyDev_Menu")+"    -    " + Localize("#"+GetActiveLevel()))
...
{
	SetupDevMenu( Localize("#Abilities"), SetDevMenu_Abilities )
	SetupDevMenu( Localize("#Equip_Weapon"), SetDevMenu_Weapons )
	SetupDevMenu( Localize("#TDMWeapon"), SetDevMenu_TDMWeapons )
	SetupDevMenu( Localize("#MDLSpawner"), SetDevMenu_ModelSpawner )

	if ( IsSurvivalMenuEnabled() )
	{
		SetupDevMenu( Localize("#ChangeCharacter"), SetDevMenu_SurvivalCharacter )
		SetupDevMenu( Localize("#OverrideSpawnCharacter"), SetDevMenu_OverrideSpawnSurvivalCharacter )
		SetupDevMenu( Localize("#Survival"), SetDevMenu_Survival )
		SetupDevMenu( Localize("#SurvivalWeapons"), SetDevMenu_SurvivalLoot, "main_weapon" )
		SetupDevMenu( Localize("#SurvivalAttachments"), SetDevMenu_SurvivalLoot, "attachment" )
		SetupDevMenu( Localize("#SurvivalHelmets"), SetDevMenu_SurvivalLoot, "helmet" )
		SetupDevMenu( Localize("#SurvivalArmor"), SetDevMenu_SurvivalLoot, "armor" )
		SetupDevMenu( Localize("#SurvivalBackpack"), SetDevMenu_SurvivalLoot, "backpack" )
		#if(false)

#endif
		SetupDevMenu( Localize("#SurvivalIncapShield"), SetDevMenu_SurvivalLoot, "incapshield" )
		SetupDevMenu( Localize("#SurvivalIncapShieldDebugging"), SetDevMenu_SurvivalIncapShieldBots )

		string itemsString = "ordnance ammo health custom_pickup"
		#if(true)
			itemsString += " data_knife"
		#endif
		SetupDevMenu( Localize("#SurvivalItems"), SetDevMenu_SurvivalLoot, itemsString )

		SetupDevCommand( Localize("#SurvivalLootZonePreprocess"), "script_ui Dev_CommandLineAddParm( \"-survival_preprocess\", \"\" ); reload" )
	}
	...
	SetupDevMenu( Localize("#RespawnPlayers"), SetDevMenu_RespawnPlayers )
	SetupDevMenu( Localize("#SetRespawnBehaviourOverride"), SetDevMenu_RespawnOverride )
	...
	SetupDevCommand( Localize("#ToggleNoClip"), "noclip" )
	SetupDevCommand( Localize("#ToggleModelViewer"), "script thread ToggleModelViewer()" )
	SetupDevCommand( Localize("#StartSkydive"), "script thread SkydiveTest()" )
	SetupDevCommand( Localize("#SpawnDeathbox"), "script thread SURVIVAL_CreateDeathBox(gp()[0], false)" )
	...
	SetupDevCommand( Localize("#DoF_Debug_Ads"), "script_client ToggleDofDebug()" )
	...
	SetupDevCommand( Localize("#SummonPlayersToPlayer0"), "script summonplayers()" )
	...
	SetupDevCommand( Localize("#ToggleSkyboxView"), "script thread ToggleSkyboxView()" )
	SetupDevCommand( Localize("#ToggleHUD"), "script thread ToggleHud()" )
	...
	SetupDevCommand( Localize("#TogglePainDeathSoundDebug"), "script TogglePainDeathDebug()" )
	SetupDevCommand( Localize("#JumpRandomlyForever"), "script_client thread JumpRandomlyForever()" )

	SetupDevCommand( Localize("#ToggleZeroingMode"), "script ToggleZeroingMode()" )
	SetupDevCommand( Localize("#EnableGodMode"), "script EnableDemigod( gp()[0] )" )
	SetupDevCommand( Localize("#DisableGodMode"), "script DisableDemigod( gp()[0] )" )
	SetupDevCommand( Localize("#ToggleScreenAlignmentTool"), "script_client DEV_ToggleScreenAlignmentTool()" )

	SetupDevMenu( Localize("#Prototypes"), SetDevMenu_Prototypes )
...
{
	SetupDevCommand( Localize("#RespawnMe"), "respawn" )
	SetupDevCommand( Localize("#RespawnAllPlayers"), "respawn all" )
	SetupDevCommand( Localize("#RespawnAllDeadPlayers"), "respawn alldead" )
	SetupDevCommand( Localize("#RespawnRandomPlayer"), "respawn random" )
	SetupDevCommand( Localize("#RespawnRandomDeadPlayer"), "respawn randomdead" )
	SetupDevCommand( Localize("#RespawnBots"), "respawn bots" )
	SetupDevCommand( Localize("#RespawnDeadBots"), "respawn deadbots" )
	SetupDevCommand( Localize("#RespawnMyTeammates"), "respawn allies" )
	SetupDevCommand( Localize("#RespawnMyEnemies"), "respawn enemies" )
...
{
	SetupDevCommand( Localize("#UseGamemodeBehaviour"), "set_respawn_override off" )
	SetupDevCommand( Localize("#OverrideAllowAllRespawning"), "set_respawn_override allow" )
	SetupDevCommand( Localize("#OverrideDenyAllRespawning"), "set_respawn_override deny" )
	SetupDevCommand( Localize("#OverrideAllowBotRespawning"), "set_respawn_override allowbots" )
}
...
{
	SetupDevCommand( Localize("#ToggleAkimboWithCurrentWeapon"), "script DEV_ToggleAkimboWeapon(gp()[0])" )
	SetupDevCommand( Localize("#ToggleAkimboWithHolsteredWeapon"), "script DEV_ToggleAkimboWeaponAlt(gp()[0])" )
